...
**✅ ĐÃ CHỐT (2026-09-04): đổi `replan_steps` từ 10 → 5 (tỉ lệ 0.5 so với `act_window_size=10`), khớp Diffusion Policy's Ta/Tp convention.** Đây là hằng số `REPLAN_STEPS` mới ở đầu `scripts/run_calvin_unshielded.py`/`run_calvin_shielded.py`/`render_obstacle_video.py` (KHÔNG còn đọc từ `cfg.multistep` nữa — hydra knob đó chỉ gate `MDTVAgent.step()`'s internal counter, dead code với harness này vì mình luôn gọi `forward()` trực tiếp). Không cần retrain: `replan_steps` chỉ là cách harness cắt (`chunk[:REPLAN_STEPS]`) chunk 10 bước mà model luôn trả về đủ, hoàn toàn độc lập với checkpoint — đúng cách Diffusion Policy tự thử nhiều `Ta` khác nhau trên 1 checkpoint duy nhất, không train lại.

**Hệ quả cần làm lại**: đổi `replan_steps` thay đổi quỹ đạo thực thi thật (chunk mới được sample từ state thật ở bước 5 thay vì tiếp tục "mù" tới bước 10) → **radius=0.08 đã tune trước đó (ở replan_steps=10) không còn valid** — `RADII_TO_SWEEP` trong `run_calvin_unshielded.py` đã reset về full list `[0.02, 0.05, 0.08, 0.12]` để sweep lại từ đầu ở `REPLAN_STEPS=5`, rồi mới sweep lại `THRESHOLDS_TO_SWEEP`/chốt τ* cho Conf-Thresh trên setup mới.
