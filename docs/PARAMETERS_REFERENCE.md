...
- **✅ ĐÃ CHỐT (real run sau fix, 2026-09-05)**: `python scripts/calibrate_arm_model_error.py num_sequences=100` (sau `CALVIN_ACTION_SCALE` fix) — `n=45632` residual, `mean=0.00223` `median=0.00187` `p10=0.00092` `p90=0.00388` `p99=0.00777` `min=0.00015` `max=0.02230` → **`model_error = p99*1.25 = 0.00972`**. Số này hợp lý (thang mm, khớp kỳ vọng cho 1 bước Jacobian-linearize), dùng được ngay làm `model_error` cho `ArmReachOnlyShield`/`ArmSTLShield`/`ArmRepairShield`/`ArmMPCFilterShield`. **Không cần chạy lại** dù `GRIPPER_TIP_OFFSET` (mục 8) sau đó bị phát hiện+sửa — `step_prediction_residual` chỉ dùng `panda_frames()` (chain 9 frame), không đụng `gripper_tip_position()`/`finger_tip_capsules()`, nên 2 bug độc lập hoàn toàn (xác nhận qua đọc lại code, không suy đoán).
...
- **⚠️ VOID toàn bộ, cần sweep lại từ đầu (2026-09-06)**: `CHOSEN_THRESHOLD=0.9` và cả bảng sweep `[0.15, 0.35, 0.6, 0.9]` phía dưới đo TRƯỚC fix `CALVIN_ACTION_SCALE` — `ArmConfThreshShield._endpoint()` gọi thẳng `propagate_arm_tube`, đúng hàm bị bug scale ~50 lần đó, nên percentile disagreement cũ (mean=0.368 median=0.349 p10=0.146 p90=0.609 p99=0.906, n=17856) đo trên 1 spread lớn hơn thật ~50 lần. Đồng thời cũng đo trước fix g(a)/self-collision/offset_max — dù 2 fix sau không đổi trực tiếp hình học của `_endpoint()`, chúng đổi cả trạng thái episode mỗi quyết định thấy, nên không thể tái dùng bảng cũ.
- **Sweep mới (2026-09-06, chưa chạy)**: `THRESHOLDS_TO_SWEEP` đã tăng từ 4 lên **8 điểm**: `[0.003, 0.007, 0.012, 0.02, 0.03, 0.15, 0.35, 0.9]`. Lý do: rescale đơn giản dự đoán percentile mới ≈ percentile cũ/50 (p10≈0.003, median≈0.007, p90≈0.012, p99≈0.018, max≈0.028) — nhưng đây chỉ là ước lượng, chưa đo thật. Để tránh lặp lại tình huống cũ (guess range sai hoàn toàn, phải chạy `--tuning` 2 lần — từng gặp cả ở threshold này lẫn epsilon của STL-Monitor), list mới cố tình phủ CẢ vùng thấp dự đoán LẪN vùng cao cũ làm lưới an toàn — dù regime nào đúng, vẫn có vài điểm rơi vào vùng có ý nghĩa ngay lần chạy đầu. `RUN_DIAGNOSTIC` đã bật lại `True` (đang `False`) để log percentile thật của chính lần chạy này, xác nhận/bác bỏ ước lượng /50 trên trực tiếp.
- **Việc cần làm khi có số thật**: đọc percentile Phase 1 (RUN_DIAGNOSTIC) trước, nếu lệch nhiều so với ước lượng /50, có thể cần chỉnh `THRESHOLDS_TO_SWEEP` một lần nữa trước khi tin tưởng bảng sweep Phase 2 — nhưng với 8 điểm trải rộng 2.5 bậc độ lớn, khả năng cao lần chạy đầu này đã đủ để thấy 1 đường cong thật (không bão hoà toàn bộ ở 1 đầu).
...
- **✅ Đã có bản cho tay máy (2026-09-06)**: `shortstop/arm_shield.py::ArmCBFShield` — QP 1-bước (chỉ sửa `candidates[0][0]`, không đụng phần còn lại của chunk, khác hẳn `ArmMPCFilterShield` sửa cả H-bước), điều kiện CBF rời rạc `n_i·dp_i + alpha*h_i(q_hiện_tại) >= 0` với `h_i` = margin signed-distance giống mọi shield khác (không dùng dạng bình phương như bản 2D). `select()`/`resolve()` là CÙNG 1 QP (CBF không có "horizon còn lại" để re-plan) — khác `ArmMPCFilterShield` (resolve() mới là chỗ làm nó thành receding-horizon thật). Script: `scripts/run_calvin_cbf.py`, sweep `ALPHAS_TO_SWEEP=[0.3,0.5,1.0,2.0,4.0]` (placeholder), `CHOSEN_ALPHA=1.0` (placeholder, chưa sweep thật). Test: `tests/test_arm_shield.py` (8 test, gồm 1 test xác nhận `alpha` thật sự kiểm soát mức phục hồi margin khi đã ở trạng thái không an toàn).
...
**Chấp nhận 2026-09-06 (user)**: `replan_steps > 1` (CALVIN dùng 10) được chấp nhận rõ ràng như **1 điểm khác biệt có chủ đích so với paper**, không phải thứ cần sửa cho khớp `replan_steps=1` — dùng đúng thuật ngữ "`replan_steps > 1`" khi mô tả deviation này trong báo cáo/paper, không mập mờ là "gần giống protocol thật". Conf-Thresh (không có `recertify`/`resolve`) chịu ảnh hưởng trực tiếp nhất từ deviation này (mù trong suốt cửa sổ 10 bước); các shield có `recertify`/`resolve` giảm nhẹ tác động (xem bảng "tach tan suat filter khoi policy" bên dưới) nhưng không loại bỏ hoàn toàn deviation này.
...
**Chấp nhận 2026-09-06 (user)**: đây là quyết định CÓ Ý ĐỒ, dùng để cover/giải thích cho việc chấp nhận `replan_steps > 1` (mục ngay trên) — Conf-Thresh chịu giới hạn `freq_filter=freq_policy` NHƯ MỘT ĐẶC THÙ CỦA CHÍNH CƠ CHẾ NÓ, không phải lỗi cấu hình cần sửa. Không cần thêm hành động gì.
...
**✅ Đã build 2026-09-06: `ArmCBFShield`** — đúng như dự đoán ở trên, cùng bản chất QP 1-bước như MPC-Filter, nhưng CHỈ 1 bước duy nhất (không phải cả H-bước) và `select()`/`resolve()` là CÙNG 1 lệnh QP (không có "phần còn lại" để re-plan riêng như MPC-Filter). Xem mục "alpha (CBF-Shield gain)" ở trên cho chi tiết + script.
...
- **4 sphere-điểm không đủ để check va chạm dọc theo link** (câu hỏi user đặt ra trực tiếp): elbow/forearm/wrist chỉ là 1 ĐIỂM + bán kính cắt ngang tại 1 đầu của link, nhưng link đó thật ra dài 0.14–0.35m (đo từ mesh) — obstacle nằm giữa link (không trúng đúng điểm sphere) sẽ bị bỏ sót. Thêm nữa, link0/1/2/4/6 không có sphere nào đại diện cả. **Đã sửa cho ground-truth check** (`calvin_experiment._clearance`): giờ dùng `robot_geometry.capsule_segments()` — 8 capsule (đoạn thẳng + bán kính) nối đủ 9 frame (0..8), phủ toàn bộ chiều dài tay. **✅ ĐÃ SỬA cho `arm_reach.py`'s reachtube (dùng bởi Certify step của shield thật)** — không còn chỉ check 4 điểm cũ nữa, `propagate_arm_tube` giờ certify đủ 9 frame + 8 link capsule, đúng chain ground-truth dùng (xem `arm_reach.py`'s docstring "RESOLVED (was 'KNOWN MISMATCH'...)"). Gap còn lại chỉ ở vùng ngón tay/fingertip — xem mục "Category A.2/A.4" bên dưới.
...
**⚠️⚠️ BUG THỨ 2 phát hiện 2026-09-05 (real re-sweep sau khi fix bug scale action ở trên) — CẢ BẢNG TRÊN ĐỀU VÔ HIỆU, cần sweep lại lần nữa, lý do KHÁC hẳn**:
real run `RADII_TO_SWEEP=[0.0, 0.02, 0.04, 0.08, 0.12, 0.16]` cho kết quả **100% subtask-đầu-tiên violate ngay, GIỐNG HỆT NHAU ở r=0.0 và r=0.02** (`violation_rate` báo `0.2` chỉ vì `build_fixed_cohort_slots` tính 1 slot violated/5 slot = 0.2, thực chất là 100% attempt-1 violate, `steps_taken` chỉ 1-8/10). Root cause: `sample_obstacle_from_reference_chunk` đặt obstacle CHÍNH XÁC tại điểm cuối reachtube dự đoán của flange — điểm này nằm ĐÚNG trên đường tâm quỹ đạo mà `finger_tip_capsules()` (vươn `GRIPPER_TIP_OFFSET=0.14m` phía trước flange, đúng hướng di chuyển) chắc chắn quét qua khi tay tiến gần đích của chính nó — biến obstacle.radius thành vô nghĩa (bán kính capsule tay, 0.06-0.20m, một mình đã đủ "bắt" chắc chắn). Bug đầu (action-scale) từng VÔ TÌNH che giấu vấn đề này (đặt obstacle xa hơn thật ~50 lần, ngẫu nhiên tạo ra đủ sai lệch dự đoán-thực tế để có phân bố hit/miss hợp lý) — giờ sửa xong, obstacle đặt đúng vị trí thật, bug nền tảng này mới lộ ra.

**Đã sửa (`shortstop/calvin_obstacle.py::sample_obstacle_from_reference_chunk`)** — 2 cơ chế độc lập, không dùng `model_error`/`w_bar` làm nguồn random (2 số đó là ngân sách hiệu chỉnh của shield, cố ý giữ nhỏ/chặt, không phải nguồn nhiễu benchmark — dùng chung sẽ khiến 1 lần calibrate lại vô tình đổi độ khó benchmark):
1. `horizon_multiplier` (mặc định 2, placeholder) — tile `reference_chunk` lặp lại N lần trước khi `propagate_arm_tube`, kéo dài quãng đường dự đoán dọc THEO CÙNG HƯỚNG (không cần gọi thêm `propose()`, không phá RNG sync với baseline unshielded) — trực tiếp fix "chưa kịp di chuyển đã violate" (thêm real steps_taken runway).
2. `offset_max=0.3` + `rng` (bắt buộc truyền tường minh, KHÔNG lấy từ numpy global RNG để tránh phá nhiễu diffusion của policy) — random 1 offset trong mặt phẳng VUÔNG GÓC hướng di chuyển, làm `radius` có ý nghĩa trở lại (không phải luôn 100% hoặc luôn 0% tùy offset cố định). Seed theo `sequence_seed_base+idx` (per-sequence, không phải per-radius) — cùng 1 sequence dùng cùng 1 offset ngẫu nhiên ở mọi radius trong sweep, so sánh apples-to-apples.

Cả `horizon_multiplier=2` và `offset_max=0.3` đều là **placeholder chưa qua sweep thật** — cần chạy lại `RADII_TO_SWEEP` với cơ chế mới này để xem đường cong violation_rate có hợp lý (không floor/ceiling ở mọi giá trị) trước khi chốt bất cứ con số nào. Toàn bộ 6 script driver (`run_calvin_unshielded.py`/`run_calvin_shielded.py`/`run_calvin_stl_monitor.py`/`run_calvin_mpc_filter.py`/`run_calvin_shortstop.py`/`run_calvin_shortstop_ablation.py`) đã cập nhật để truyền `rng` seed theo sequence — kể cả phần re-run video debug (rebuild `rng` riêng theo đúng `vis_idx` để video khớp đúng placement đã đo, không dùng lại rng đã bị "tiêu" từ vòng lặp chính).
...
**Quyết định 2026-09-06 (user)**: KHÔNG cần sửa logic sinh obstacle (không cần đổi `frame_index`) — `offset_max` + `horizon_multiplier` đã tạo đủ random để obstacle đôi khi rơi gần elbow/forearm một cách tự nhiên (không cố tình nhắm vào đó, nhưng không loại trừ). Điều kiện CẦN là **checker va chạm phải detect được** khi việc đó xảy ra — đã xác nhận qua code: `_clearance()` (`calvin_experiment.py`) luôn lặp qua `capsule_segments(joint_angles)` (đủ 8 link, toàn bộ chiều dài tay, không chỉ flange) cho MỌI lần gọi, bất kể obstacle được sample gần frame nào — nên va chạm elbow/forearm với obstacle (dù hiếm, do obstacle thường được neo gần flange) vẫn được phát hiện đúng nếu nó thực sự xảy ra. Không cần code change.
...
**Chốt lần đầu: radius = 0.08** (balance-point reasoning: `attempted_violation_fraction=0.487` gần đúng "50/50", `violation_rate`/`success_rate` cohort đều 0.184) — **sau đó ĐỔI LẠI cùng ngày, xem mục ngay dưới.**

**⚠️ ĐỔI LẠI 2026-09-06 (cùng ngày): radius = 0.08 → 0.06 — LÝ DO KHÁC HẲN (không phải floor/ceiling)**: sau khi bàn về `ArmRepairShield` (`trust_region=0.05m`, `max_repair_iters=1` — chỉ 1 lần đẩy trajectory tối đa `trust_region` để thoát vi phạm, khớp Algorithm 1), nhận ra vùng "bắt" tổng ở radius=0.08 (cộng thêm bán kính capsule tay 0.06-0.20m) có thể lên tới ~0.28m — lớn hơn nhiều so với ngân sách sửa 1-lần của Repair (0.05m). Một candidate vi phạm sâu hơn `trust_region` thì về mặt toán học **không thể** thoát trong 1 bước push, bất kể hướng gradient đúng hay sai. Hạ radius xuống 0.06 giảm bớt rủi ro "quá khó sửa" này cho ShortStop/Repair, mà không đổi tới `trust_region` (tham số riêng của shield, không phải benchmark).

**Lưu ý về quy trình**: quyết định 0.06 chạy TRỰC TIẾP trên **eval cohort** (idx 100-199, không có sweep riêng ở đúng 0.06 trên tuning cohort trước — 0.06 nằm giữa 0.04 và 0.08 đã sweep ở bảng trên, nội suy hợp lý chứ không phải giá trị hoàn toàn mới lạ) — đây là 1 lần đi tắt có chủ đích khỏi quy trình `feedback_tuning_cohort_split` thường lệ, ghi nhận rõ ở đây chứ không giấu đi.

**Số liệu final (eval cohort, idx 100-199, `results/unshielded/run.log`)**: `violation_rate=0.186`, `success_rate=0.222`, `avg_seq_len=1.11/5`, `attempted_violation_fraction=0.447` (93/208 attempted subtasks), `min_clearance` mean=0.068 median=0.0202 p10=-0.0656 p90=0.2348 min=-0.1476 max=0.4240. **Đây là radius CUỐI CÙNG dùng cho MỌI baseline CALVIN** (`RADII_TO_SWEEP`/`OBSTACLE_RADIUS` đã đồng bộ ở cả 8 nơi: 6 script driver + `run_calvin_full_eval.py` + `sample_obstacle_from_reference_chunk`'s own default).

**⚠️ Bug công bằng phát hiện + sửa 2026-09-06** (câu hỏi trực tiếp của user: "Conf-Thresh và các baseline khác có apply cách đặt obs này không?"): `OBSTACLE_OFFSET_MAX=0.6` chỉ được thêm vào `run_calvin_unshielded.py` khi tăng từ 0.3 — **5 script còn lại** (`run_calvin_shielded.py`/Conf-Thresh, `run_calvin_stl_monitor.py`, `run_calvin_mpc_filter.py`, `run_calvin_shortstop.py`, `run_calvin_shortstop_ablation.py`) vẫn gọi `sample_obstacle_from_reference_chunk` KHÔNG truyền `offset_max`, nên âm thầm dùng default cũ của hàm (0.3) — nghĩa là 5 baseline có shield đang bị test với obstacle "khó" hơn hẳn (offset nhỏ hơn, dễ chạm hơn) so với baseline unshielded đang dùng để so sánh, dù cùng ghi `radius=0.08`. **Không công bằng nếu chạy thật lúc chưa sửa.** Đã sửa: thêm `OBSTACLE_OFFSET_MAX = 0.6` làm hằng số tường minh vào cả 5 script (giống cách `OBSTACLE_RADIUS` đã làm), truyền tường minh ở cả 2 lambda (main loop + video debug) mỗi script, và in ra `obstacle_offset_max` trong dòng log config. Đồng thời cập nhật default của chính hàm `sample_obstacle_from_reference_chunk` (`shortstop/calvin_obstacle.py`) từ 0.3 → 0.6 (không còn placeholder, đã confirm qua sweep) để giảm rủi ro lặp lại lỗi này ở script tương lai — nhưng vẫn giữ nguyên tắc mỗi script phải khai báo tường minh `OBSTACLE_OFFSET_MAX` riêng (không dựa vào default của hàm), đúng tinh thần "mọi tham số ảnh hưởng độ khó benchmark phải nhìn thấy được ngay tại chỗ gọi, không giấu trong 1 default dùng chung". `render_obstacle_video.py` không bị ảnh hưởng (không truyền `rng`, nên cơ chế offset chưa từng áp dụng ở đó).
...
| Class | Param | Default |
|---|---|---|
| ArmReachOnlyShield | `w_bar`, `model_error` | (truyền vào) / **0.00972 (real, xem mục 1 "model_error")** |
| ArmSTLShield | +`epsilon` | 0.02 |
| ArmRepairShield | +`trust_region`, `step_size`, `max_repair_iters` | 0.05 / 0.02 / 1 |

**Cập nhật 2026-09-05**: dòng "chưa có script đo residual thật" bên dưới đã STALE — `scripts/calibrate_arm_model_error.py` đã có và đã chạy thật, xem mục 1's "model_error" entry cho số liệu + `model_error=0.00972` đã chốt.
...
- **A.4**: `_clearance()`/`_candidate_clearance()` (ground truth, `calvin_experiment.py`) giờ dùng `finger_tip_capsules(joint_angles, gripper_width)` — 2 capsule ngón tay THẬT, lấy `gripper_width` thật từ `obs["robot_obs_raw"][6]` (`gripper_opening_width`, xác nhận qua chính code `calvin_env.robot.Robot.get_observation`), thay vì luôn giả định mở hết cỡ (`GRIPPER_TIP_RADIUS=0.06` cũ).
- **✅ Đã sửa phần SHAPE cho reachtube của shield (2026-09-06)** — trước đó `propagate_arm_tube` gộp cả vùng ngón tay vào 1 quả cầu bảo thủ tại flange (`FRAME_RADIUS[FLANGE_FRAME_INDEX] = GRIPPER_TIP_OFFSET+GRIPPER_TIP_RADIUS = 0.20m`) — **không sai an toàn** (bao trọn capsule ngón tay thật theo bất đẳng thức tam giác, mọi điểm trong capsule cách flange tối đa đúng 0.20m) nhưng thừa bảo thủ theo hướng ngón tay không bao giờ vươn tới (vd. ngay bên hông flange). Đã thêm 1 capsule "fingertip" thật (flange → `gripper_tip_position()`, bán kính `model_error+w_bar+GRIPPER_TIP_RADIUS`) vào `propagate_arm_tube`, đồng thời frame 8 (flange) tự nó đổi sang dùng `LINK_RADIUS[-1]=0.06` (khớp cách các frame khác tính bán kính riêng) thay vì `FRAME_RADIUS[8]` — nếu không đổi cả 2 chỗ này cùng lúc thì quả cầu cũ (to hơn) vẫn "che" mất lợi ích của capsule mới (do `arm_robustness_to_go` lấy min qua mọi capsule). **Vẫn CHƯA track `gripper_width` động** (bán kính cố định `GRIPPER_TIP_RADIUS`, không đọc `gripper_width` thật theo từng bước dự đoán) — quyết định phạm vi có chủ đích y hệt trước: threading `gripper_width` qua toàn bộ tương lai dự đoán của 1 candidate cần mô phỏng động lực học cơ cấu kẹp, đổi interface lớn hơn hẳn lợi ích thu được (chặn trên đã nhỏ, 0.06m). Chỉ áp dụng cho `arm_reach.py`'s reachtube (Certify STL) — `ArmMPCFilterShield`'s CBF/QP riêng (`arm_shield.py`, đọc thẳng `FRAME_RADIUS[i]`, không qua `propagate_arm_tube`) chưa đụng tới, ngoài phạm vi fix này. 4 test cũ hard-code `FRAME_RADIUS[FLANGE_FRAME_INDEX]`/offset cụ thể đã cập nhật theo số liệu thật (verify qua script trước khi sửa assertion, không đoán) — 161/161 test pass.
...
- **Quyết định 2026-09-06 (user)**: đây là hạn chế của **thực nghiệm** (implementation cụ thể cho CALVIN), trong khi **lý thuyết** (CESearch/reachtube) đang cố chứng minh tổng quát cho $X_u$ bất kỳ hình dạng — không phải mâu thuẫn, chỉ là thực nghiệm chưa phủ hết phạm vi lý thuyết cho phép. Có thể bổ sung box/plane sau nếu cần một stress-test khó hơn; không phải việc cần làm ngay.
