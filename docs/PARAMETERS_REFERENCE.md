...
**Sửa lại 2026-09-05**: 2 dòng dưới đây trước đó claim "đã verify khớp URDF thật" cho `PANDA_DH` — **claim đó không chính xác/stale**, và bảng này còn trích dẫn API cũ (`SPHERE_RADII`/`SPHERE_FRAME_INDICES`) không còn tồn tại trong code (đã thay bằng `LINK_RADIUS`/`FRAME_RADIUS`/`capsule_segments()`, phủ toàn bộ 8 link chứ không chỉ 4 điểm named). Cái "verify" thật sự đã làm chỉ là so tay vài hằng số DH (d/a) với `<origin>` của URDF — không phải so sánh FK/pose thật. `shortstop/robot_geometry.py`'s docstring của chính nó vẫn đúng khi tự nhận "chưa chạy với robot/sim thật".
...
| `PANDA_DH` | bảng modified-DH | **Chưa thật sự verify với FK/pose thật** — chỉ so tay vài constant với URDF `<origin>`. `scripts/verify_robot_geometry_against_pybullet.py` (mới 2026-09-05) so trực tiếp `panda_frames()` với `p.getLinkState()` của chính sim CALVIN đang chạy — **chưa chạy** (cần WSL2), đây mới là cách verify thật. |
